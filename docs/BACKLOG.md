...
- [ ] **Default language drift.** Pages init to **EN** at runtime, but SITE-CONFIG says default `es`. Toggle wiring also differs per page (`L()`+`be/bs` on gorras; `btEN/btES` on stickers; `setLang()`+`b-en/b-es` elsewhere). Now *parameterised* in `header.njk` rather than copy-pasted, so unifying it is a much smaller change than before — but it still alters runtime behaviour, so treat as its own pass.
